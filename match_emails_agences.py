...
def is_placeholder(email: str) -> bool:
    """Détecte les adresses factices aspirées par erreur (ex: adresse@adresse.fr,
    email@email.com, nom@nom.fr) : la partie locale reprend le nom de domaine
    lui-même, ce qui n'arrive jamais sur une vraie adresse nominative — c'est
    quasi toujours un placeholder/exemple présent sur le site scrapé."""
    local, _, domain = email.partition("@")
    if not domain:
        return False
    sld = domain.split(".", 1)[0]
    nl, ns = norm(local), norm(sld)
    return bool(nl) and len(ns) >= 3 and nl == ns
...
def index_scraped(rows: list[dict[str, Any]]) -> dict[str, list[str]]:
    ...
    for row in rows:
        ...
        emails = {e for e in (clean_email(x) for x in raw) if e and not is_placeholder(e)}
...
def base_result(row: dict[str, str], index: dict[str, list[str]]) -> tuple[dict[str, Any], bool]:
    ...
    if existing:
        ...
        # Rien trouvé du tout pour cette agence : la clé de recherche (nom /
        # domaine du CSV) ne correspond à rien dans le fichier scrapé. C'est
        # un désaccord entre les deux listes, pas une vraie ambiguïté.
        note = "aucun candidat : nom/domaine de l'agence introuvable dans le fichier scrapé"
    elif method == "none":
        note = "emails trouvés mais tous génériques, aucun nominatif disponible"
    else:
        note = ""
    result = {
        ...
        "matching": {"methode": method, "score": score, "note": note},
        "matched_at": datetime.now(timezone.utc).isoformat(),
    }
    return result, needs_ai


AMBIGUOUS_METHODS = {"ambiguous", "none", "unmatched", "unmatched_no_live_key", "cerebras_error"}


def write_ambiguous_report(path: Path, results: list[dict[str, Any]]) -> int:
    """Exporte en CSV les agences sans e-mail de gérant fiable, pour relecture
    manuelle : vraies ambiguïtés (plusieurs candidats plausibles) et désaccords
    entre les deux listes sources (aucun candidat retrouvé)."""
    rows = [r for r in results if r["matching"]["methode"] in AMBIGUOUS_METHODS]
    with path.open("w", encoding="utf-8", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Agence", "Gérant", "E-mails candidats", "Méthode", "Note"])
        for r in rows:
            writer.writerow([
                r["Agence"], r["Gérant"],
                "; ".join(r["tous_les_emails_trouves"]),
                r["matching"]["methode"], r["matching"]["note"],
            ])
    return len(rows)
...
def main() -> None:
    ...
    parser.add_argument("--ambiguous-output", default="agences_ambigues.csv")
    ...
    write_output(output_path, results)
    n_ambiguous = write_ambiguous_report(Path(args.ambiguous_output), results)
    print(f"Terminé: {len(rows)} agences, {len(tasks)} cas ambigus, {len(keys)} clé(s) Cerebras détectée(s) -> {args.output}")
    print(f"{n_ambiguous} agence(s) à relire manuellement -> {args.ambiguous_output}")
